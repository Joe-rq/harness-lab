...
├── scripts/                  # CLI 工具和验证脚本
├── skills/                   # 通用治理技能与阶段导航
├── .claude/commands/         # 可调用的 skills（/harness-setup）
...
这个 skill 会引导你完成交互式安装，自动检测冲突、选择模块、配置 hooks。

**使用 CLI 脚本**：

```bash
# 克隆或下载 harness-lab
git clone https://github.com/Joe-rq/harness-lab.git

# 在目标项目中运行安装脚本
cd your-project
node /path/to/harness-lab/scripts/harness-install.mjs
