...
会话启动输出固定分三区：「机器状态」来自事件投影（唯一真相源），「最近事件」是最近 8 条事件流水与总条数，「人的笔记」原文照登 `.claude/progress.txt`。`Last updated` 只由工作事件推进，`session_started` / `session_ended` 不计——打开会话不等于有进展；`phase` 只表示当前活跃 REQ 的阶段，无活跃 REQ 即 `idle`，"被阻塞"由搁置列表表达。`requirements/INDEX.md` 的搁置列表改为逐行扫描渲染，不再因正则捕获组丢掉首项。
