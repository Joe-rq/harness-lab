...
其中 `docs:impact` / `docs:verify` 会先读取当前 git working tree / staged / untracked 状态，再按 `scripts/docs-sync-rules.json` 的规则约束“哪些脚本 / 协议改动通常必须联动更新哪些入口文档”。
推荐顺序是先看 `npm run docs:impact`，再改文档，最后让 `npm run req:complete -- --id REQ-...` 通过收尾。
...
- 如果改动新增了脚本、命令或关键文档引用，请先运行 `npm run docs:impact`，再运行 `npm run docs:verify`
