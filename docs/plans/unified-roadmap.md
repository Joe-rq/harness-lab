...
#### 2B.1 req-cli --type 参数化 + 特化 slash command

```
修改：scripts/req-cli.mjs（新增 --type 参数 + 4 个构建函数）
简化：.claude/commands/bugfix.md（薄壳：收集信息 → req:create --type）
简化：.claude/commands/feature.md（薄壳）
简化：.claude/commands/refactor.md（薄壳）
```

**设计原则**：
- **模板源统一为 req-cli.mjs 一个源头**——`buildReqContent()` 按 `--type` 路由到特化构建函数（`buildBugfixReqContent` / `buildFeatureReqContent` / `buildRefactorReqContent`），消除 prompt 保证的不可靠性
- slash command 退化为**薄壳**（收集信息 → `req:create --type` → Edit 补占位符），核心逻辑在代码中
- 通用模板（无 `--type`）保持向后兼容
- 所有类型补全缺失章节（颗粒度自检、反馈与质量检查）

**各类型特化内容**：

| 字段 | --type bugfix | --type feature | --type refactor |
|------|---------|----------|-----------|
...
| 文件 | 操作 | 预计行数 |
|------|------|---------|
| scripts/req-cli.mjs | 修改（+4 构建函数 + buildCommonSections） | +220 |
| .claude/commands/bugfix.md | 简化为薄壳 | ~60 |
| .claude/commands/feature.md | 简化为薄壳 | ~60 |
| .claude/commands/refactor.md | 简化为薄壳 | ~60 |
...
## Phase 5: 自恢复与自治（看门狗 + 自恢复指令 + 续传 + 模式实质化 + 部署守卫 + 不变量清理）

**目标**：AI 可以长时间（>1 小时）无人干预地运行，遇异常自动选择恢复策略，而非仅注入提醒后等待 AI 自行判断。

### 为什么是第五步

前四步构建了学习闭环、安全边界和自审能力。但检测到问题后只是 inject 一句话——AI 看到后做什么完全取决于上下文。第五步的核心不是"更多护栏"，而是**让护栏产生确定性动作**：检测到停滞 → 触发恢复流程，而非检测到停滞 → 注入"你停滞了"。

### 修订说明（2026-04-27）

原 Phase 5 有三个问题：
1. **Context Router 和 Deploy Guard 是护栏打磨，不解决"AI 卡住了怎么办"**——优先级低于自恢复
2. **autonomous 模式是空壳**——三种模式唯一区别是 hook 提醒语气，没有行为分化
3. **不变量系统已膨胀至 256 个文件且大量重复**——缺少清理机制

修正方向：将"自驱动"拆解为"自恢复"（AI 遇错能自救）而非"自启动"（AI 自己取任务开始跑）。后者与治理哲学矛盾——REQ 强制、Scope Guard、Stop Evaluator 的价值在于让 AI 的工作透明可审计，自启动会让这些变成橡皮图章。

### 要构建的东西

#### 5.1 看门狗（Watchdog）✅ 已完成

```
新建：scripts/watchdog.mjs
运行方式：PostToolUse hook + CLI --diagnose
```

**逻辑**：
1. 每次文件操作后检查 progress.txt 和 REQ 状态
2. 检测停滞：同一 REQ 编辑超 10 次无阶段推进
3. 检测循环：同一 REQ 状态反复切换超 3 次
4. 超阈值 → 通过 additionalContext 注入提醒

**完成状态**：REQ-2026-047，已实现、测试、推送。

#### 5.2 自恢复指令

```
修改：CLAUDE.md + 新增 .claude/commands/resume.md
```

**问题**：Watchdog 检测到停滞后只注入"你停滞了"，AI 看到后行为不确定。

**逻辑**：
1. CLAUDE.md 新增"异常响应协议"章节：当 AI 收到停滞/循环/风险提醒时的标准响应流程
2. 标准响应流程：
   - 停滞 → ① 自检是否可拆分子任务 → ② 可拆分则创建新 REQ 拆分 → ③ 不可拆分则标记 blocked + 记录原因
   - 循环 → ① 确认阻塞根因 → ② 记录到 REQ 的"阻塞/搁置说明" → ③ 尝试换方法（最多 1 次）→ ④ 仍循环则标记 blocked
   - 风险 R3+ → ① 先运行 `npm test` → ② 失败则标记 blocked + 记录失败信息
3. 新增 `/resume` slash command：读取 progress.txt + session-log + watchdog-state → 建议下一步行动
4. **本质是 prompt 层面的决策指南**，非新脚本——hook 架构决定了 hook 只能 inject，决策逻辑必须在 AI 的行为指令里

#### 5.3 上下文续传

```
修改：scripts/session-start.sh + CLAUDE.md
```

**问题**：SessionStart 展示状态但不主动恢复；SessionEnd 写摘要但下次不自动拾取。

**逻辑**：
1. session-start.sh 增强：检测到"Last active REQ ≠ none"且 REQ 仍在 in-progress → 展示中断点和上次编辑的文件
2. CLAUDE.md 新增"续传协议"：AI 启动时发现有未完成 REQ → 主动询问"是否继续 REQ-XXX？"
3. 不自动继续——需要人确认。autonomous 模式除外（见 5.4）

#### 5.4 autonomous 模式实质化

```
修改：各 hook 脚本（watchdog、stop-evaluator、scope-guard、risk-tracker）
```

**问题**：三种模式唯一区别是提醒语气，没有行为分化。

**逻辑**：
| 场景 | collaborative | supervised | autonomous |
|------|--------------|------------|------------|
| Watchdog 停滞 | 提醒 + 建议恢复策略 | 阻断 + 强制选择恢复策略 | 静默执行恢复策略 + 记录到日志 |
| Stop 未覆盖 | 提醒 | 阻断 | 阻断（autonomous 也不允许假完成） |
| Scope 越界 | 提醒 | 阻断 | 阻断（autonomous 也不允许越界） |
| Risk R3+ | stderr 提醒 | 阻断 + 提醒 | 允许 + 记录到 .risk-actions.log |
| 续传 | 询问是否继续 | 询问是否继续 | 自动继续 + 记录 |

**核心原则**：
- collaborative = 人确认后执行
- supervised = 人确认后执行，不确认则阻断
- autonomous = 自动执行 + 事后可审计
- **安全边界不可绕过**：Stop 未覆盖和 Scope 越界在任何模式下都阻断
...
**逻辑**：
1. 拦截危险 Bash 命令（rm -rf、force push、drop table、git reset --hard）
2. collaborative → 提醒
3. supervised → 阻断
4. autonomous → 阻断（自恢复能力越强，刹车越重要）

**为什么保留**：5.2-5.4 让 AI 有了自恢复能力，AI 可能尝试越来越激进的手段。Deploy Guard 是防止自恢复变成自毁的最后一道墙。

#### 5.6 不变量清理

```
修改：scripts/invariant-extractor.mjs + scripts/invariant-gate.mjs
```

**问题**：context/invariants/ 已膨胀至 256 个文件，多次 --scan 产生大量重复 INV。

**逻辑**：
1. 去重：--scan 时检测标题+触发路径相同的 INV，合并而非新建
2. 使用频率追踪：被 req-check --check 触发的 INV 记录触发次数到 .claude/.inv-usage.json
3. 长期未触发（90 天）的 INV 自动标记为 deprecated
4. 高频触发的 INV 升级严重级别（medium → high）

### 退出标准

- [x] 看门狗能检测并报告 REQ 停滞（✅ REQ-2026-047）
- [ ] AI 收到停滞/循环提醒后能执行标准恢复流程（非随机行为）
- [ ] SessionStart 展示中断点 + AI 主动询问是否继续
- [ ] 三种模式有行为分化（不只是语气差异）
- [ ] 部署守卫拦截危险命令
- [ ] 不变量去重 + 频率追踪 + 自动废弃运转
...
| 文件 | 操作 | 预计行数 |
|------|------|---------|
| scripts/watchdog.mjs | 已完成 | ~195 |
| CLAUDE.md | 修改（自恢复指令+续传协议） | +40 |
| .claude/commands/resume.md | 新建 | ~20 |
| scripts/session-start.sh | 修改（中断点展示） | +15 |
| scripts/stop-evaluator.mjs | 修改（模式分化） | +10 |
| scripts/scope-guard.mjs | 修改（模式分化） | +5 |
| scripts/risk-tracker.mjs | 修改（模式分化+autonomous日志） | +15 |
| scripts/watchdog.mjs | 修改（模式分化） | +15 |
| scripts/deploy-guard.mjs | 新建 | ~100 |
| scripts/invariant-extractor.mjs | 修改（去重+频率） | +50 |
| scripts/invariant-gate.mjs | 修改（自动废弃） | +20 |
...
```
               ...
               ──────────     ──────────      ──────────      ──────────      ──────────      ──────────      ──────────      ──────────      ──────────
Hook 覆盖：
  SessionStart ✅ 状态恢复    ✅ +不变量统计   ✅              ✅ +模式显示     ✅              ✅              ✅              ✅ +中断点展示   ✅
  PreToolUse   ✅ REQ +不变量 ✅ +注入扩展     ✅              ✅              ✅ +范围强制     ✅ +审查隔离    ✅ +部署守卫    ✅
  PostToolUse  ✅ 循环检测    ✅              ✅              ✅              ✅ +风险追踪     ✅              ✅ +看门狗      ✅
  Stop         ❌            ❌              ❌              ✅ 完成度评估    ✅              ✅              ✅              ✅ 模式分化     ✅
  SessionEnd   ❌            ❌              ❌              ✅ 自动反思      ✅              ✅              ✅              ✅              ✅
  PreCompact   ❌            ❌              ❌              ❌              ❌              ❌              ❌              ❌              ✅

学习闭环：
  不变量提取    ✅ 基础扫描    ✅ 质量门禁     ✅              ✅              ✅              ✅              ✅              ✅ 去重+频率    ✅ 跨项目
  不变量注入    ✅ req-check   ✅ 多节点注入   ✅              ✅              ✅              ✅              ✅              ✅ +自动废弃    ✅
...
| Phase 5 | 5-6 个 REQ（5.1 已完成） | 2-3 个/REQ | Phase 4 真实项目验证后 | Phase 3A+4；5.2 依赖 5.1；5.4 依赖 5.2 |
