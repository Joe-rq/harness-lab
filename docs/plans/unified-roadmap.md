...
人机协作    →   经验回流深化   →   DX + 参数化     →   安全护栏       →   自审查         →   全自治         →   跨项目共享
(已完成)        (学习闭环)        (降低摩擦)        (防假+防越界)     (审查隔离)        (自学习)          (终极形态)
人当守门人      经验→规则→注入    30分钟可上手      脚本守门          AI自审+QA         看门狗+学习       脱敏共享
```
...
**目标**：不同类型的 REQ（bugfix/feature/refactor）有各自特化的创建模板。
...
```
新建：skills/req/bugfix.md
新建：skills/req/feature.md
新建：skills/req/refactor.md
```
...
```
新建：skills/adapters/react.md
新建：skills/adapters/python.md
新建：skills/adapters/go.md
```

**逻辑**：针对常见项目类型提供预设配置（hook 参数、文件路径模式、测试命令等）。
...
1. 运行 `req:create --type bugfix`，检查模板自动包含"复现步骤"字段
2. 运行 `req:create --type feature`，检查模板自动包含"用户故事"字段
3. 运行 `req:create --adapter python`，检查自动填充 Python 相关配置

### 预计文件改动

| 文件 | 操作 | 预计行数 |
|------|------|---------|
| skills/req/bugfix.md | 新建 | ~60 |
| skills/req/feature.md | 新建 | ~60 |
| skills/req/refactor.md | 新建 | ~60 |
| skills/adapters/react.md | 新建 | ~40 |

---

## Phase 3: 安全护栏（防假完成 + 防越界）

**目标**：AI 不能再说"我搞定了"就结束，也不能改 REQ 声明范围外的文件。

### 为什么是第三步

经验回流和 DX 就绪后，框架已有足够的"学习力"和"易用性"。此时构建安全护栏，能让 AI 的行为被约束在安全范围内，为后续自审查和全自治打下基础。
...
信任边界：
  Layer 1 认知 人            人+规则          人+规则          人              脚本            脚本+不变量     共享不变量
  Layer 2 意图 人+脚本       人+脚本          人+脚本          脚本(拦截)      脚本(隔离)      脚本(全守门)    脚本
  Layer 3 执行 人+脚本       人+脚本          人+脚本          脚本(范围)      脚本(自审)      脚本(看门狗)    脚本
  Layer 4 优化 人            脚本(提取)       人              人              人              脚本(学习)      脚本(共享)
...
| Phase 2A | 1 个 REQ | 2-3 个/REQ | Phase 1 完成后 | 无强依赖 |
| Phase 2B | 1-2 个 REQ | 2-3 个/REQ | Phase 2A 验证后 | Phase 2A（向导可用后参数化才有意义） |
| Phase 3 | 3-4 个 REQ | 2-3 个/REQ | Phase 2B 验证后 | 无强依赖，但建议先完成 Phase 1-2 |
| Phase 4 | 3-4 个 REQ | 2-3 个/REQ | Phase 3 稳定后 | Phase 3（范围强制是审查的前提） |
| Phase 5 | 4-5 个 REQ | 2-3 个/REQ | Phase 4 真实项目验证后 | Phase 3+4 |
| Phase 6 | 待定 | 待定 | Phase 5 稳定运行后 | Phase 5 |

**关键原则**：
- Phase 1、2A、2B 可交错推进，互不强依赖
- Phase 3 是 Phase 4 的硬前置
...
| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| 不变量质量低，注入变成噪音 | AI 忽略提醒 | draft/active 生命周期，人工审核 |
...
| 路线图 Phase 1（Stop 评估器） | Phase 3.2 | 优先级下调 |
| 路线图 Phase 2（范围+风险） | Phase 3.4-3.5 | 与 Stop 合并为"安全护栏" |
...
| 元反思方向 4（Stop 评估器） | Phase 3.2 | 与路线图 Phase 1 合并 |
