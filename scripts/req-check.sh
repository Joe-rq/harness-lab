...
if [ -n "$ACTIVE_REQ" ] && [ "$ACTIVE_REQ" != "none" ] && [ "$ACTIVE_REQ" != "无" ]; then
  ...
  if [ -z "$REQ_FILE" ] || [ ! -f "$REQ_FILE" ]; then
    node "$ROOT/scripts/error-classifier.mjs" --type REQ_NOT_FOUND --context "reqId=$ACTIVE_REQ"
    exit 2
  fi
...
node "$ROOT/scripts/error-classifier.mjs" --type NO_ACTIVE_REQ --context "file=$TARGET_FILE"
exit 2
