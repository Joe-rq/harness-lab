...
if [ -n "$ACTIVE_REQ" ] && [ "$ACTIVE_REQ" != "none" ] && [ "$ACTIVE_REQ" != "无" ]; then
  ...
  if [ -n "$REQ_IN_PROGRESS" ]; then
    ...
    # 根据模式决定续传行为
    if [ "$HARNESS_MODE" = "autonomous" ]; then
      echo "  🔄 autonomous 模式：自动继续 ${ACTIVE_REQ}（已记录到日志）"
      # 记录自动续传到 watchdog-actions.log
      echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) | 续传：自动继续 ${ACTIVE_REQ} (autonomous mode)" >> "$ROOT_DIR/.claude/.watchdog-actions.log" 2>/dev/null
    else
